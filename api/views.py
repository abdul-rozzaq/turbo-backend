...
from .serializers import ShopLoginSerializer, ShopSerializer, TokenSerializer
...
def login(request):
    ...
    if setializer.is_valid():
        shop = Shop.objects.get(name=setializer.data['name'], password=setializer.data['password'])
        token = Token.objects.create(shop=shop, device=setializer.data['device'])
